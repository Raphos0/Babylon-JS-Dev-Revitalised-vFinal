...
export async function createCharacterController(scene: Scene) {
  ...
  // Make capsule invisible (keeps collider shape for debugging if needed)
  displayCapsule.isVisible = false;

  // Create physics character controller (keep as before)
  ...
  try {
      const result = await SceneLoader.ImportMeshAsync("", "./assets/hunter/", "golem.gltf", scene);
      // Create a single transform root to hold all imported meshes
      ...
      // Optionally reposition/rotate/scale the visual root to match capsule origin
      ...
      visualRoot.position.y -= bb.min.y; // example tweak

      // If the glTF contains a skeleton, you can start animations:
      // if (result.skeletons && result.skeletons.length) { scene.beginAnimation(result.skeletons[0], 0, 100, true); }
    } catch (e) {
      console.error("Failed to import character model:", e);
    }
